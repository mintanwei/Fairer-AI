...
parser = argparse.ArgumentParser(description='PyTorch FairerOPTH Training on MixNAF')
parser.add_argument('--gpuid', type=str, default='0,1')
parser.add_argument('--seed', type=str, default=30)
...
model_save_dir = os.path.join(os.getcwd(), 'saved_models', 'MixNAF')
...
def main():
    ...
    # initiate dataset
    Train_dataset = MixNAF_TrainDataset(data_dir=config.MixNAF_dir, transform=config.transform_train)
...
def train_multi_label_coco(model, train_loader, val_loader, lr, log):
    ...
    for epoch in range(Epochs):
        ...
        mAP_score = validate_multi(val_loader, model, log)
        model.train()
...
def validate_multi(val_loader, model, log):
